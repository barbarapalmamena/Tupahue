...
export async function uploadMinisterioImagen(file) {
    try {
        console.log("Iniciando subida vía API:", file.name);
        const client = getSupabaseClient();
        ...
        formData.append('bucket', 'ministerios');
        ...
        if (!response.ok) {
            throw new Error(result.error || 'Error en el servidor al subir imagen');
        }

        console.log("Subida exitosa vía API:", result.publicUrl);
        return { data: { publicUrl: result.publicUrl }, error: null };
    } catch (err) {
        console.error("Excepción en uploadMinisterioImagen (API):", err);
